...
export class InsertQuery<T> extends BaseQuery<T> implements IInsertQuery<T> {
  ...
  doUpdate(value: { excluded?: DoUpdateExcludeValues<T>; merge?: Partial<T> }): this {
    if (this._onConflictClause) {
      throw new Error("On conflict clause already set");
    }
    ...
    if (value.excluded && value.merge) {
      ...
      const duplicateKeys = Object.keys(value.merge).filter((key) => excludedKeys.has(key));
      if (duplicateKeys.length > 0) {
        throw new Error(`Properties ${duplicateKeys.join(", ")} cannot be set in both excluded and merge objects`);
      }
    }
