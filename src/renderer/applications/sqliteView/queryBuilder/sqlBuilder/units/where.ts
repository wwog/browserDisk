...
import { isWhereConditionDescription, mergeLikePatterns, quotes, validateBindings } from "../../utils";
...
const optimizeSet = (
  ...
  const targetKey = `${key}_${type}` as Exclude<Keyof<PartItem>, "hasAndCondition">;
  ...
  if (key === "$between") {
    if (!Array.isArray(sourceValue) || sourceValue.length !== 2) {
      throw new Error(`BETWEEN condition for column "${column}" must be an array with exactly 2 values`);
    }
    ...
    switch (key) {
      ...
      case "$in":
        ...
        if (!Array.isArray(sourceValue)) {
          throw new Error(`IN condition for column "${column}" must be an array`);
        }
        if (target["$in_OR"]?.length) {
      ...
      case "$between":
        if (target["$between_OR"]?.length) {
          ...
          target["$between_OR"] = [[Math.min(sourceValue[0], existing[0]), Math.max(sourceValue[1], existing[1])]];
          return;
        }
        break;
  ...
  if (type === "AND") {
    ...
    if (highPriority.includes(key) && target.hasAndCondition) {
      ...
      throw new Error(`"${column}" has already had another condition with equality in AND condition`);
    }
...
const convertToSQL = (optimizeResult: PreprocessResult): SQLWithBindings => {
  ...
  if (conditionParts.size > MAX_IN_VALUES) {
    throw new Error(`Too many conditions (${conditionParts.size}). Maximum allowed is ${MAX_IN_VALUES}`);
  }
  ...
  conditionParts.forEach((partItem, column) => {
    ...
    (Object.keys(operatorMap) as (keyof BasePartItem)[]).forEach((operator) => {
  ...
  const andConditionsStr = conditions.length > 0 ? conditions.map((c) => c.sql).join(" AND ") : "";
  const orConditionsStr = orConditions.length > 0 ? orConditions.map((c) => c.sql).join(" OR ") : "";
  const allBindings = [...conditions.flatMap((c) => c.bindings), ...orConditions.flatMap((c) => c.bindings)];
